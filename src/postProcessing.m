...
if (settings.IQ == 1)
    [fid_I, ~] = fopen(settings.fileName_I, 'rb');
    [fid_Q, message] = fopen(settings.fileName_Q, 'rb');
    fid = [fid_I, fid_Q];
else
    [fid, message] = fopen(settings.fileName_I, 'rb');
end

%If success, then process the data
if (sum(fid > 0) == size(fid, 2))
    
    % Move the starting point of processing. Can be used to start the
    % signal processing at any point in the data record (e.g. good for long
    % records or for signal processing in blocks).
    if (settings.IQ == 1)
        fseek(fid(1), settings.skipNumberOfBytes, 'bof');
        fseek(fid(2), settings.skipNumberOfBytes, 'bof');
    else
...
%% Acquisition ============================================================
    ...
    if ((settings.skipAcquisition == 0) || ~exist('acqResults', 'var'))
        ...
        if (settings.IQ == 1)
            data_I = fread(fid(1), 11*samplesPerCode, settings.dataType)';
            data_Q = fread(fid(2), 11*samplesPerCode, settings.dataType)';
...
%% Plot all results ===================================================
    ...
    error('Unable to read file %s: %s.', settings.fileName_I, message);
end % if (fid > 0)
