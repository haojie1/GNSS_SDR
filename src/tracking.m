...
for channelNr = 1:settings.numberOfChannels
    ...
    if (channel(channelNr).PRN ~= 0)
        ...
        if (settings.IQ == 1)
            fseek(fid(1), ...
              settings.skipNumberOfBytes + (channel(channelNr).codePhase-1)*settings.dataLength, ...
              'bof');
          fseek(fid(2), ...
              settings.skipNumberOfBytes + (channel(channelNr).codePhase-1)*settings.dataLength, ...
              'bof');
...
%% Read next block of data ------------------------------------------------            
            ...
            % interation 
            if (settings.IQ == 1)
                [rawSignal_I, ~] = fread(fid(1), ...
                                             [1, blksize], settings.dataType);
                [rawSignal_Q, samplesRead] = fread(fid(2), ...
                                             [1, blksize], settings.dataType);
                rawSignal = rawSignal_I + 1j*rawSignal_Q;                         
            else
...
%% Generate the six standard accumulated values ---------------------------
            ...
            if settings.IQ == 1
                qBasebandSignal = real(rawSignal .* (carrCos + 1j*carrSin));
                iBasebandSignal = imag(rawSignal .* (carrCos + 1j*carrSin));
            else
...
%               %* Modify
%               % Check if this is the first time to cauculate the frequency error
% 	if firstCauculate==1
%         firstCauculate = 0;
%         carrError = 0;
%         carrNco = carrFreq;
...
%         prevQ = Q_P;
%     else
%         carrError = atan2((I_P*prevI+Q_P*prevQ),(Q_P*prevI+I_P*prevQ))/(2*pi)/PDIcarr;
%         fllOutput = fllOutput + 40*carrError*PDIcarr;
%         carrFreq = fllOutput + carrFreqBasis;
%         carrNco = carrFreq;
%         prevI = I_P;
%         prevQ = Q_P;
%     end
...
%% Record various measures to show in postprocessing ----------------------
            ...
            if (settings.IQ == 1)
                trackResults(channelNr).absoluteSample(loopCnt) = ftell(fid(1))/settings.dataLength;
            else
                trackResults(channelNr).absoluteSample(loopCnt) = ftell(fid)/settings.dataLength;
            end   
